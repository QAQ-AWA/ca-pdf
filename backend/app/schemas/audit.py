...
class AuditLogEntry(BaseModel):
    ...
    actor_id: int | None = Field(
        default=None, description="User responsible for the event"
    )
    event_type: str = Field(description="Event type identifier")
    resource: str = Field(description="Name of the resource associated with the event")
    ip_address: str | None = Field(
        default=None, description="IP address recorded for the event"
    )
    user_agent: str | None = Field(
        default=None, description="User agent associated with the event"
    )
    meta: dict[str, Any] | None = Field(
        default=None, description="Structured metadata payload"
    )
