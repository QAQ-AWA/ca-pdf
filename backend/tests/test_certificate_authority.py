...
@pytest.mark.anyio
async def test_issue_certificate_returns_valid_pkcs12_bundle(
    client: AsyncClient,
) -> None:
    admin_token = await _login(
        client=client, email=ADMIN_EMAIL, password=ADMIN_PASSWORD
    ...
    user = await _create_user(
        email=user_email, password=user_password, role=UserRole.USER
    )
...
async def test_revoke_certificate_and_generate_crl(client: AsyncClient) -> None:
    ...
    assert any(
        entry["artifact_id"] == crl_payload["artifact_id"] for entry in crl_entries
    )
...
async def _ensure_root(*, client: AsyncClient, admin_token: str) -> None:
    ...
    if response.status_code not in {201, 409}:
        pytest.fail(
            f"Unexpected response creating root CA: {response.status_code} {response.text}"
        )
...
async def _create_user(*, email: str, password: str, role: UserRole) -> User:
    ...
    async with session_factory() as session:
        return await create_user(
            session=session, email=email, password=password, role=role
        )
...
async def _build_external_pkcs12_bundle() -> tuple[bytes, str]:
    ...
    subject = x509.Name(
        [x509.NameAttribute(NameOID.COMMON_NAME, "Imported Certificate")]
    )
    builder = (
